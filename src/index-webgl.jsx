...
// WebGL Video Player Component with Custom Controls using Three.js
const WebGLVideoPlayer = ({ isOpen, onClose }) => {
  ...
  React.useEffect(() => {
    ...
    // Robust video initialization with multiple retry attempts
    const initVideo = () => {
      console.log(
        `🎬 Initializing video (attempt ${retryCount + 1}/${maxRetries})...`
      );
      ...
      if (!canvas || !canvas.getContext) {
        ...
        if (retryCount < maxRetries) {
          ...
          setVideoError("Failed to initialize canvas after multiple attempts");
        }
        return;
      }
      ...
      if (videoRef.current) {
        console.log("🎬 Video already exists, skipping initialization");
        return;
      }
      ...
      console.log("🎬 Created video element with src:", video.src);

      // Test video URL accessibility
      fetch("/vidoes/video.mp4", { method: "HEAD" })
        .then((response) => {
          if (response.ok) {
        ...
        .catch((error) => {
          ...
          setVideoError("Error checking video file");
        });
      ...
      // Add a test plane to verify WebGL is working
      const testGeometry = new THREE.PlaneGeometry(0.1, 0.1);
      const testMaterial = new THREE.MeshBasicMaterial({ color: 0xff0000 });
      const testMesh = new THREE.Mesh(testGeometry, testMaterial);
      testMesh.position.set(0.8, 0.8, 0);
      scene.add(testMesh);
      ...
      // Add video to DOM (hidden) so it can load
      document.body.appendChild(video);
      console.log("🎬 Video added to DOM");

      // Test video loading with a simple approach
      ...
      // Video event listeners with comprehensive debugging
      video.addEventListener("loadstart", () => {
        console.log("🎬 Video: loadstart - Video loading started");
      });

      video.addEventListener("loadeddata", () => {
        console.log("🎬 Video: loadeddata - First frame loaded");
        setVideoLoaded(true);
      ...
      video.addEventListener("canplaythrough", () => {
        console.log(
          "🎬 Video: canplaythrough - Can play through without stopping"
        );
        setVideoLoaded(true);
      });
      ...
      video.addEventListener("play", () => {
        console.log("🎬 Video: play - Video started playing");
        setIsPlaying(true);
      });
      ...
      video.addEventListener("stalled", () => {
        console.warn("⚠️ Video: stalled - Video loading stalled");
      });

      video.addEventListener("waiting", () => {
        console.warn("⚠️ Video: waiting - Video waiting for data");
      });

      video.addEventListener("abort", () => {
        console.warn("⚠️ Video: abort - Video loading aborted");
      });

      video.addEventListener("emptied", () => {
        console.warn("⚠️ Video: emptied - Video element emptied");
      });

      video.addEventListener("suspend", () => {
        console.warn("⚠️ Video: suspend - Video loading suspended");
      });
      ...
      const loadingTimeout = setTimeout(() => {
        if (!videoLoaded) {
          ...
          setVideoError("Video loading timeout - please retry");
        }
      }, 10000); // 10 second timeout

      // Robust video loading with multiple strategies
      const loadAndPlayVideo = async () => {
        ...
        const attemptVideoLoad = async () => {
          try {
            videoLoadAttempts++;
            console.log(
              `🎬 Loading video (attempt ${videoLoadAttempts}/${maxVideoLoadAttempts})...`
            );
            ...
            console.error(
              `❌ Video load attempt ${videoLoadAttempts} failed:`,
              error
            );
        ...
        try {
          await attemptVideoLoad();
        } catch (error) {
          console.error("❌ All video load attempts failed:", error);
          clearTimeout(loadingTimeout);
          setVideoError(
            "Failed to load video after multiple attempts - please retry"
          );
        }
      };
      ...
      // Animation loop with texture updates
      const animate = () => {
        animationIdRef.current = requestAnimationFrame(animate);

        // Always update video texture when video is ready
        if (video.readyState >= 2) {
          videoTexture.needsUpdate = true;
        }

        // Force texture update every frame for smooth playback
      ...
      const cleanup = () => {
        ...
        if (video) {
          video.pause();
          // Don't clear src to avoid empty src error
          // video.src = "";
    ...
    // Use a single, well-timed initialization
    ...
    return (
      ...
      >
        ...
        >
          ...
          {/* Same controls overlay as WebGL version */}
          <div
            style={{
              position: "absolute",
              bottom: "0",
          ...
          >
            ...
            {/* Control Buttons - Same as WebGL version */}
            <div
              style={{
                display: "flex",
                alignItems: "center",
          ...
          >
            ...
            <button
              onClick={() => {
                ...
                if (videoRef.current) {
                  // Ensure video has src before loading
                  if (!videoRef.current.src) {
                    videoRef.current.src = "/vidoes/video.mp4";
                  }
